...
function deleteROI(id) {
  this.get('wsx').invoke('delete_roi', id);
}
function upsertROI(roi, ...props) {
  const baseProps = ['id'];
  ...
  this.get('wsx').invoke('upsert_roi', data).then(data => {
    ...
    // roi.setProperties({id: data.id}, data.attributes);
  });
}
export default {
  didTransition() {
